"use client";

import { useState } from "react";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { Spinner } from "@/components/ui/spinner";
import { cn } from "@/lib/utils";
import type { GitProvider } from "@/features/git-providers/lib/oauth-config";

interface ProviderConnectCardProps {
  provider: GitProvider;
  connected: boolean;
  username?: string | null;
}

const PROVIDER_INFO = {
  github: {
    name: "GitHub",
    icon: <GitHubIcon />,
    description: "Connect your GitHub account",
  },
  gitlab: {
    name: "GitLab",
    icon: <GitLabIcon />,
    description: "Connect your GitLab account",
  },
  bitbucket: {
    name: "Bitbucket",
    icon: <BitbucketIcon />,
    description: "Connect your Bitbucket account",
  },
};

export function ProviderConnectCard({
  provider,
  connected,
  username,
}: ProviderConnectCardProps) {
  const info = PROVIDER_INFO[provider];
  const [isConnecting, setIsConnecting] = useState(false);

  const handleConnect = () => {
    setIsConnecting(true);
    // Redirect to OAuth flow with onboarding return URL
    window.location.href = `/api/git/connect/${provider}?returnTo=/onboarding/connect`;
  };

  return (
    <div
      className={cn(
        "relative flex flex-col items-center gap-4 rounded-xl border p-6 transition-all",
        connected
          ? "border-primary/50 bg-primary/5"
          : "border-border bg-card hover:border-primary/30 hover:bg-accent/50"
      )}
    >
      {connected && (
        <Badge
          variant="secondary"
          className="absolute -top-2.5 bg-primary text-primary-foreground"
        >
          Connected
        </Badge>
      )}

      <div
        className={cn(
          "flex size-16 items-center justify-center rounded-2xl",
          connected ? "bg-primary/10 text-primary" : "bg-muted text-foreground"
        )}
      >
        {info.icon}
      </div>

      <div className="text-center">
        <h3 className="font-semibold">{info.name}</h3>
        <p className="mt-1 text-sm text-muted-foreground">
          {connected && username ? `@${username}` : info.description}
        </p>
      </div>

      {!connected && (
        <Button
          onClick={handleConnect}
          disabled={isConnecting}
          variant="outline"
          className="w-full"
        >
          {isConnecting ? (
            <>
              <Spinner className="size-4" />
              Connecting...
            </>
          ) : (
            <>Connect</>
          )}
        </Button>
      )}
    </div>
  );
}

function GitHubIcon() {
  return (
    <svg className="size-8" viewBox="0 0 24 24" fill="currentColor">
      <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
    </svg>
  );
}

function GitLabIcon() {
  return (
    <svg className="size-8" viewBox="0 0 24 24" fill="currentColor">
      <path d="M22.65 14.39L12 22.13 1.35 14.39a.84.84 0 0 1-.3-.94l1.22-3.78 2.44-7.51A.42.42 0 0 1 4.82 2a.43.43 0 0 1 .58 0 .42.42 0 0 1 .11.18l2.44 7.49h8.1l2.44-7.51A.42.42 0 0 1 18.6 2a.43.43 0 0 1 .58 0 .42.42 0 0 1 .11.18l2.44 7.51L23 13.45a.84.84 0 0 1-.35.94z" />
    </svg>
  );
}

function BitbucketIcon() {
  return (
    <svg className="size-8" viewBox="0 0 24 24" fill="currentColor">
      <path d="M.778 1.213a.768.768 0 0 0-.768.892l3.263 19.81c.084.5.515.868 1.022.873H19.95a.772.772 0 0 0 .77-.646l3.27-20.03a.768.768 0 0 0-.768-.891zM14.52 15.53H9.522L8.17 8.466h7.561z" />
    </svg>
  );
}
